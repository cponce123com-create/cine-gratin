...
export default function Events() {
  ...
  return (
    <div className="min-h-screen bg-brand-dark pt-20 pb-16">
      <Helmet>
        <title>Eventos — Cine Gratín</title>
      </Helmet>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm">
            {events ? `${filtered.length} eventos disponibles` : "Cargando..."}
          </p>
        </div>
        ...
        {selectedEvent && (
          <div className="mb-12 bg-black rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10">
            <div className="aspect-video w-full relative">
              <iframe
                src={`https://www.youtube.com/embed/${selectedEvent}?autoplay=1`}
            ...
            <div className="p-4 bg-brand-surface/50 flex justify-between items-center">
              <h2 className="text-lg font-bold text-white">
                {events?.find(e => e.yt_id === selectedEvent)?.title}
              </h2>
              <button 
        ...
        {error && (
          <div className="text-center py-20">
            <p className="text-red-400 text-lg">No se pudieron cargar los eventos.</p>
          </div>
        )}
            ...
            {filtered.map((event) => (
              ...
              >
                <div className="aspect-video w-full rounded-lg overflow-hidden bg-brand-surface card-hover relative">
                  <img
                    src={event.thumbnail || FALLBACK_THUMBNAIL}
                    alt={event.title}
                    loading="lazy"
                ...
                <div className="mt-3">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-gray-500 font-medium">{event.channel_name}</span>
                    {event.published_at && (
