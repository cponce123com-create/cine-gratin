...
export default function ManageMovies() {
  ...
  const toggleSelectAllPage = () => {
    if (allOnPageSelected) {
      setSelected((prev) => {
        ...
        paginated.forEach((m) => n.delete(m.id));
        return n;
      });
    } else {
      setSelected((prev) => {
        const n = new Set(prev);
        paginated.forEach((m) => n.add(m.id));
        return n;
      });
    }
  };
  ...
  return (
    <AdminLayout>
              ...
              <tbody className="divide-y divide-brand-border">
                {loading ? (
                  ...
                  paginated.map((m) => (
                    <tr
                      key={m.id}
                      className={`hover:bg-brand-surface/50 transition-colors ${selected.has(m.id) ? "bg-brand-red/5" : ""}`}
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            className={`p-1.5 rounded transition-colors ${m.featured ? "text-brand-gold bg-brand-gold/10" : "text-gray-500 hover:text-brand-gold hover:bg-brand-gold/10"}`}
                          >
      ...
      {editingItem && (
        <EditMediaModal item={editingItem} type="movie" onClose={() => setEditingItem(null)} onSaved={load} />
      )}
    </AdminLayout>
  );
}
