...
export default function ManageSeries() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [series, search, genreFilter, yearFilter, missingFilter, sortKey, sortAsc]);
  ...
  const toggleFeatured = async (s: Series) => {
    try {
      await saveSeries({ id: s.id, featured: !s.featured });
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-white mb-1">Gestionar Series</h1>
            <p className="text-gray-500 text-sm">
              {filtered.length} de {series.length} series
            </p>
          </div>
